...
\begin{earg}
\item[\ex{if5}] Se alguém quiser me ver, estarei na varanda.\footnote{Em português, assim como no inglês original, esta construção condicional é frequentemente usada para expressar uma declaração incondicional sobre onde a pessoa estará, daí ser traduzida simplesmente como $P$.}
\end{earg}
...
A linguagem-objeto neste capítulo é LS. A metalinguagem é o inglês matemático — aqui vertido para o português, mas ainda suplementado com vocabulário lógico e matemático. A expressão `$(A\eor B)$' é uma sentença na linguagem-objeto, porque usa apenas símbolos de LS. Já a palavra `sentença' não faz parte de LS; assim, a frase `Esta expressão é uma sentença de LS' não é uma sentença de LS. É uma sentença na metalinguagem, usada para falar \emph{sobre} LS.

Nesta seção, daremos uma definição formal de `sentença de LS'. A definição será dada em inglês matemático (metalinguagem), aqui traduzido para o português.

...
Uma vez que usamos letras sentenciais separadas para $N$ e $E$, apagamos qualquer conexão entre as duas. Elas passam a ser apenas duas sentenças atômicas que podem ser verdadeiras ou falsas de maneira independente. Em inglês, é impossível que ao mesmo tempo ninguém e todo mundo esteja confuso. Como sentenças de LS, porém, existe uma atribuição de valores de verdade para a qual $N$ e $E$ são ambas verdadeiras.
...
Esse argumento é \emph{inválido} em LS. (Novamente, você pode confirmar isso com uma tabela-verdade.) Há algo de muito errado aqui, porque em inglês esse é claramente um argumento válido. A simbolização em LS deixou de fora toda a estrutura importante. Mais uma vez, a tradução para LS ignora a estrutura quantificacional: a sentença `Todos os lógicos usam chapéus engraçados' fala tanto de lógicos quanto do fato de usar chapéus engraçados. Ao não traduzir essa estrutura, perdemos a conexão entre o fato de Willard ser lógico e o fato de Willard usar chapéu.
...
De forma semelhante, se uma sentença com quantificadores sai como uma \emph{tautologia em LS}, então a sentença em inglês é logicamente verdadeira. Se ela sai como \emph{contingente em LS}, isso pode ser devido à estrutura quantificacional que é perdida quando traduzimos para a linguagem formal.
...
Em inglês, um \define{termo singular} é uma palavra ou expressão que se refere a uma pessoa, lugar ou coisa \emph{específica}. A palavra `cachorro' não é um termo singular, porque há muitos cachorros. A expressão `o cachorro Bertie do Philip' é um termo singular, porque se refere a um terrierzinho específico.
...
Os predicados mais simples são propriedades de indivíduos. Eles são coisas que você pode dizer sobre um objeto. `\blank\ é um cachorro' e `\blank\ é um integrante do Monty Python' são ambos predicados. Ao traduzir sentenças do inglês, o termo nem sempre aparece no começo da sentença: `Um piano caiu sobre \blank' também é um predicado. Predicados como esses são chamados de \define{monádicos} ou \define{de um lugar}, porque há apenas um espaço em branco a ser preenchido. Um predicado monádico e um termo singular se combinam para formar uma sentença.
...
Para eliminar essa ambiguidade, precisaremos especificar um \define{universo de discurso} — abreviado UD. O UD é o conjunto das coisas sobre as quais estamos falando. Se quisermos falar de pessoas em Chicago, definimos o UD como sendo `pessoas em Chicago'. Escrevemos isso no início da chave de simbolização, assim:
\begin{ekey}
\item[UD:] pessoas em Chicago
\end{ekey}
Os quantificadores \emph{varrem} o universo de discurso. Dado esse UD, $\forall x$ significa `Todo mundo em Chicago' e $\exists x$ significa `Alguém em Chicago'. Cada constante nomeia algum membro do UD, então só podemos usar esse UD com a chave de simbolização acima se Donald, Gregor e Marybeth estiverem todos em Chicago. Se quisermos falar de pessoas em outros lugares além de Chicago, precisamos incluí-las no UD.
...
\end{earg}
Essa tradução é um desastre! Ela transforma o que, em inglês, é um argumento péssimo em um argumento válido em LQ. O problema é que há uma diferença entre ser \emph{habilidoso como cirurgião} e ser \emph{habilidoso como tenista}. Traduzir esse argumento corretamente exige dois predicados distintos, um para cada tipo de habilidade. Se deixarmos $K_1x$ significar `x é habilidoso como cirurgião' e $K_2x$ significar `x é habilidoso como tenista', então podemos simbolizar o argumento desta forma:
\begin{earg}
...
\solutions
\problempart
\label{pr.LQalligators}
Usando a chave de simbolização dada, traduza cada sentença em inglês para LQ.
\begin{ekey}
...
\problempart Usando a chave de simbolização dada, traduza cada sentença em inglês para LQ.
\begin{ekey}
\item[UD:] todos os animais
...
\problempart
\label{pr.LQcandies}
Usando a chave de simbolização dado, traduza cada sentença em inglês para LQ.
\begin{ekey}
\item[UD:] doces
...
\problempart
Usando a chave de simbolização dada, traduza cada sentença em inglês para LQ com identidade. A última sentença é ambígua e pode ser traduzida de duas maneiras; você deve fornecer ambas as traduções. (Dica: a identidade só é necessária para as quatro últimas sentenças.)
\begin{ekey}
\item[UD:] pessoas
...
\problempart
\label{pr.LQcards}
Usando a chave de simbolização dada, traduza cada sentença em inglês para LQ com identidade. As duas últimas sentenças são ambíguas e podem ser traduzidas de duas maneiras; você deve fornecer ambas as traduções para cada uma delas.
\begin{ekey}
...
\problempart Usando a chave de simbolização dada, traduza cada sentença em inglês para LQ com identidade. As duas últimas sentenças são ambíguas e podem ser traduzidas de duas maneiras; você deve fornecer ambas as traduções para cada uma delas.
\begin{ekey}
